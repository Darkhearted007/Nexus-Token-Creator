...
const JITO_TIP_ACCOUNT = new PublicKey(
  "96g9sAg9u3m9TW2bsf3qcGYK3SGu3qc7pEdPspUX6qc5"
);
...
export async function runSniperBot(mintAddress: string, tierCost: number) {
  ...
  console.log(
    `[Sniper Bot] ⚡ Deploying ${walletsToUse.length} sub-wallets for target acquisition...`
  );
  ...
  const executeSnipe = async (wallet: Keypair, index: number) => {
    try {
      ...
      if (balance < 0.02 * LAMPORTS_PER_SOL) {
        console.warn(
          `[Sniper Bot] ⚠️ Wallet ${index + 1} (${wallet.publicKey
            .toBase58()
            .slice(0, 6)}) has insufficient funds.`
        );
        return;
      }
      ...
      console.log(
        `[Sniper Bot] 🛡️ Wallet ${index + 1} - Protection instructions built.`
      );
  ...
  console.log(
    `[Sniper Bot] 🛡️ Block-0 protection engaged for ${mintAddress}. Standing by for liquidity events.`
  );
}
