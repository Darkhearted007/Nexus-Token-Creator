...
export function validateTokenForm(form: TokenFormValues): string[] {
  ...
  else if (name.length > 64) errors.push('Token Name must be 64 characters or less.');

  const symbol = form.symbol.trim();
  if (!symbol) errors.push('Ticker Symbol is required.');
  else if (symbol.length > 12) errors.push('Ticker Symbol must be 12 characters or less.');
  else if (!/^[A-Za-z0-9_]+$/.test(symbol)) errors.push('Ticker Symbol can only contain letters, numbers, and underscores.');

  if (!Number.isInteger(form.decimals) || form.decimals < 0 || form.decimals > 18)
